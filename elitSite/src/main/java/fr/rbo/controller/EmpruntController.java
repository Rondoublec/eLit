...
public class EmpruntController {
    ...
    private static final Logger log = LoggerFactory.getLogger(EmpruntController.class);
    private String referer = "emprunts";
     ...
     * @return
     */
    @GetMapping({"/emprunts", "/lesemprunts"})
    public String Topo(Model model, HttpSession httpSession, HttpServletRequest request) {
        ...
        if (url.equals("/lesemprunts")){
            referer = "emprunts";
        }
        Topo topoCriteres = new Topo();
        if (referer.equals("mesemprunts")) {
            User user = recupUser();
    ...
    /**
     * Affiche la liste des topos de l'user connecté
     * @param model
     * @param httpSession
     * @return
     */
     ...
     * @return
     */
    @PostMapping(value = "/emprunts/recherche")
     ...
     * @return
     */
    @GetMapping("/emprunts/delete/{topoId}")
     ...
     * @return
     */
    @GetMapping("/emprunts/demande/{topoId}")
    public String EmprunterTopo(@PathVariable("topoId") Long topoId, final RedirectAttributes redirectAttributes) {
        ...
        if (topoServiceInterface.emprunterTopo(topoId, user)){
     ...
     * @return
     */
    @GetMapping("/emprunts/accepte/{topoId}")
     ...
     * @return
     */
    @GetMapping("/emprunts/annule/{topoId}")
     ...
     * @return
     */
    @GetMapping("/emprunts/edit/{topoId}")
     ...
     * @return
     */
    @GetMapping("/emprunts/affiche/{topoId}")
    ...
    private boolean estPersonnel (User user) {
        ...
        if (user != null) {
            ...
            for (Role role : roles) {
                if (role.getRole().equals("PERSONNEL")) { personnel = true; }
            }
        }
        return personnel;
    }
    ...
    private String pageOrigine() {
        String page = "redirect:/emprunts";
        if (referer == "mesemprunts") { page = "redirect:/" + referer; }
        return page;
    }
